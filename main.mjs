...
input.addEventListener('change', e => {
    ...
    setCameraAndModel(scene)
    renderer.render(camera, model);
});

function setCameraAndModel(scene) {    
...
let keys = []
let currentlyMoving = model
let speed = 0.1
document.addEventListener('keydown', (event) => {
    var name = event.key;
    // change what you are moving (model/camera)
    if(event.key === '1') currentlyMoving = model
    else if(event.key === '2') currentlyMoving = camera
    if(currentlyMoving == camera) speed = 0.04
    else speed = 0.1
...
document.addEventListener('keypress', (event) => {
    // rotations
    if(keys['a']) currentlyMoving.rotation[1] -= speed
    if(keys['d']) currentlyMoving.rotation[1] += speed
    if(keys['w']) currentlyMoving.rotation[0] -= speed
    if(keys['s']) currentlyMoving.rotation[0] += speed
    if(keys['q']) currentlyMoving.rotation[2] -= speed
    if(keys['e']) currentlyMoving.rotation[2] += speed
    // translations
    if(keys['i']) currentlyMoving.translation[1] += speed
    if(keys['k']) currentlyMoving.translation[1] -= speed
    if(keys['j']) currentlyMoving.translation[0] -= speed
    if(keys['l']) currentlyMoving.translation[0] += speed
    if(keys['u']) currentlyMoving.translation[2] += speed
    if(keys['o']) currentlyMoving.translation[2] -= speed
    // scale
    if(keys['t']) currentlyMoving.scale[1] -= speed
    if(keys['g']) currentlyMoving.scale[1] += speed
    if(keys['f']) currentlyMoving.scale[0] += speed
    if(keys['h']) currentlyMoving.scale[0] -= speed
    if(keys['r']) currentlyMoving.scale[2] -= speed
    if(keys['y']) currentlyMoving.scale[2] += speed
    // perspective
    if(keys['p'] && currentlyMoving === camera) currentlyMoving.perspective += 1
    if(keys[';'] && currentlyMoving === camera) currentlyMoving.perspective -= 1
